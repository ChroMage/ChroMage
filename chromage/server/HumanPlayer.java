...
import chromage.shared.Mage.Type;
...
public class HumanPlayer extends Thread implements Player {
     ...
     */
    public HumanPlayer(Socket socket, Server server) {
        this.socket = socket;
    ...
    private void startStateSending() {
        new RateLimitedLoop(Constants.TICKS_PER_SECOND) {
            public boolean shouldContinue() {
                return HumanPlayer.this.shouldKeepProcessing;
            }
            public void body() {
                try {
                    ...
                    server.disconnectPlayer(HumanPlayer.this);
    ...
    public UserInput getCurrentInputState() { 
        if (state.getCurrentTick() - getLastUpdateTick() > Constants.INPUT_TIMEOUT_TICKS) {
            resetCurrentInputState();
        }
